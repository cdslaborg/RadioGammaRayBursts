...
:: add detector model to preprocessor flags
if %DETECTOR%==swift (
    set FPP_FLAGS_SYNTHETIC_SAMPLE=%FPP_FLAGS_SYNTHETIC_SAMPLE% /define:SWIFT_ENABLED
) else (
    if %DETECTOR% NEQ batse (
        echo.
        echo. -- Fatal Error occurred: DETECTOR=%DETECTOR% is not recognized as an option.
        echo. -- exiting...
        echo.
        cd %~dp0
        set ERRORLEVEL=1
...
if %LTYPE%==dynamic (
    ...
    REM set SYNTHETIC_SAMPLE_EXECUTABLE_NAME=%SYNTHETIC_SAMPLE_PROG%.exe
